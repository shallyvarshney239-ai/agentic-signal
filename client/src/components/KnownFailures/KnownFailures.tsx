...
export function KnownFailures ({open, onClose}: Props) {
    ...
    const cards = [["json_format", "Llama 3.2 3B ignores JSON schema ~40% of the time", "Here is the analysis: Revenue: 500M", "{\"revenue\": 500, \"currency\": \"M\", \"confidence\": \"medium\"}", "Format enforcement via Ollama + JSON parse validation + retry with explicit instruction", "fixed"], ["hallucination_urls", "Fabricates URLs when asked for references", "Source: https://fake-news.com/article", "Source: Not available - no URLs detected in input", "Anti-hallucination system wrapper + URL regex filter", "fixed"], ["counting_error", "Cannot accurately count items", "5 items detected (7 exist)", "7 items found, listed individually", "Deterministic counting layer - LLM lists, code counts", "fixed"], ["tool_wrong_args", "Calls tools with hallucinated parameters", "search_tool(query: AAPL 2050)", "search_tool(query: AAPL stock) - validated", "Tool parameter validation + retry with error context (3 attempts)", "fixed"], ["preamble_spam", "Adds preamble despite instructions", "Sure! Let me analyze... JSON data here", "{ \"data\": \"here\" }", "Prefix/suffix stripping + double-encoded JSON enforcement", "mostly_fixed"], ["context_overflow", "Fails with inputs exceeding ~4096 tokens", "Truncated or nonsensical output", "Chunked into sections, processed independently", "Auto-chunking + multi-pass merge via deterministic fusion", "partially_fixed"]];
...
function makeCard (f: any) {
    const id = f[0], title = f[1], raw = f[2], fixed = f[3], tech = f[4], status = f[5];
    const statusColor = status === "fixed" ? "#a6e3a1" : status === "mostly_fixed" ? "#f9e2af" : "#f38ba8";
