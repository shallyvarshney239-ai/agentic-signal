// Generator script for creating TSX components
// Uses concatenation to avoid template literal issues
...
// ===== METRICS PANEL =====
const metricsDir = path.join(base, "MetricsPanel");
...
'    var _a = useState(metricsStore.getAggregateMetrics()), m = _a[0], setM = _a[1];',
'    var _b = useState(OllamaService.getInstance().getTelemetry()), t = _b[0], setT = _b[1];',
'    useEffect(function () {',
'        if (open) {',
...
'    }, [open]);',
'    if (!open) return null;',
'    var cloud = t.totalTokens > 0 ? ((t.totalTokens / 1000) * 0.00015).toFixed(4) : "0.00";',
'    var sections = [',
...
'        React.createElement("button", {',
'            onClick: function () {',
'                var r = metricsStore.exportReport();',
...
fs.writeFileSync(path.join(metricsDir, "MetricsPanel.tsx"), metricsContent);
